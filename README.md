...
# 2. 启动服务
npm start
#   或双击 Windows 下的 start.bat

# 3. 打开浏览器访问
http://localhost:3211
```

> 也可自定义端口：`PORT=8080 npm start`（需在 `.env` 中配置，详见下方）。

### 数据文件（可随时备份 / 迁移）

| 文件 | 内容 |
|------|------|
| `data/goals.json` | 人生坐标（使命/愿景/价值观）+ 全部目标 O/KR/任务 |
| `data/history.json` | 所有修改的历史变更（差分格式） |

> **隐私提醒**：这两个文件是纯 JSON，不做任何加密。如果你要**公开分享/开源本项目**，请先替换成匿名示例数据（仓库默认已通过 `.gitignore` 排除它们）。
